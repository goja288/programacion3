...
// @return int
int
penalizarPareja (Cliente* c1, Cliente* c2,int Q, int M) {
...
// @return int
int
obtenerQ (Cliente* c1, Cliente* c2, bool cabalas, int M) {
...
void
imprimirConjuntos (Cliente** C1, int cantC1, Cliente** C2, int cantC2) {
    printf("Conjunto 1\n");
    for (int i =0; i < cantC1; i++) {
        printf("Cliente %i - \n ",i);
        imprimirCliente(C1[i]);
    }

    printf("\n Conjunto 2\n");
    for (int i =0; i < cantC2; i++) {
        printf("- Cliente %i - \n ",i);
        imprimirCliente(C2[i]);
    }
} //imprimirConjuntos

void
imprimirTupla (Cliente** tupla, int cantC1) {
    for (int i = 0; i < cantC1; i++) {
        if (tupla[i] == NULL) {
            printf ("solo ");
        }
...
void
imprimirCandidatos (Compatibilidad* candidatos, int cantC2) {  //para debug
        printf ("candidatos:[");
        for (int i = 0; i < candidatos->cantidad; i++) {
                if(i != 0) {
                        printf (",");
                }
                if(candidatos->indices[i] >= cantC2) {
                        printf ("solo");
                }
                else {
                        printf ("%i", candidatos->indices[i]);
                }
        }
        printf ("]\n");
}
...
void
imprimirAsignados (bool* asignados, int cantC2) {  //para debug
        printf ("asignados:[");
    ...
    if (posC1 + 1 < cantC1) {
        ...
        for (int j = 0; j < candidatos->cantidad; j++) {
            ...
            if (estaEnC2) {
                ...
                solParcial->sumaQ       = solParcial->sumaQ + obtenerQ(C1[prox], C2[posC2], cabalas, M);;
