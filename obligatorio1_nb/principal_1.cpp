...
int main() {
	...
	///char c;
    //    char comando[256];//   = new char[BUFFERSIZE];
      //          char param1[256];
	bool fin = false;
	...
	while ( !fin ) {
		...
		//}
			...
			cin >> param1;	int dato = atoi(param1);
				agregarABB(abb,dato);
				printf("%s %i %s\n","El elemento",dato,"se ha agregado en el abb.");
			}
			else {
			/*	4 - cargar		*/
			if (strcmp (comando, "cargar") == 0) { /***************FALTA CONTROLAR SI EL ARCHIVO A CARGAR NO EXISTE********************************************************
                                                       ******************************************************************/
